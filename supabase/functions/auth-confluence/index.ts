...
Deno.serve(async (req) => {
  ...
  if (action === "connect") {
    ...
    const scopes = encodeURIComponent("search:confluence read:confluence-content.summary read:confluence-content.all read:confluence-space.summary read:page:confluence read:space:confluence");
    const state = crypto.randomUUID();
    const confluenceAuthUrl = `https://auth.atlassian.com/authorize?audience=api.atlassian.com&client_id=${CONFLUENCE_CLIENT_ID}&scope=${scopes}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${state}&response_type=code&prompt=consent`;

    return new Response(JSON.stringify({ url: confluenceAuthUrl }), {
